...
// File: @openzeppelin-4/contracts/interfaces/IERC20.sol
...
// File: @openzeppelin-4/contracts/interfaces/IERC20Metadata.sol
...
// File: contracts/BIFI/interfaces/velodrome-v2/IRouter.sol
...
// File: contracts/BIFI/interfaces/velodrome-v2/IPool.sol
...
// File: contracts/BIFI/zaps/zapInterfaces/IBayVault.sol

pragma solidity ^0.8.0;
interface IBayVault is IERC20 {
    function deposit(uint256 amount) external;
    function withdraw(uint256 shares) external;
    function want() external pure returns (address); // Bay Vault V6
    function token() external pure returns (address); // Bay Vault V5
...
// File: contracts/BIFI/zaps/libs/Babylonian.sol
...
// File: contracts/BIFI/zaps/BayVelodromeV2Zap.sol
...
contract BayVelodromeV2Zap {
    using SafeERC20 for IERC20;
    using SafeERC20 for IBayVault;

    IRouter public immutable router;
    address public immutable WETH;
    address public farm;
    uint public constant minimumAmount = 1000;

    constructor(address _router, address _WETH, address _farm) {
        router = IRouter(_router);
        WETH = _WETH;
        farm = _farm;

        require(WETH == router.weth(), 'Bay: WETH address not matching Router.weth()');
    }
    ...
    function beefInETH (address bayVault, uint256 tokenAmountOutMin, uint256 _pid) external payable {
        require(msg.value >= minimumAmount, 'Bay: Insignificant input amount');

        IWETH(WETH).deposit{value: msg.value}();

        _swapAndStake(bayVault, tokenAmountOutMin, WETH, _pid);
    }

    function beefIn (address bayVault, uint256 tokenAmountOutMin, address tokenIn, uint256 tokenInAmount, uint256 _pid) external {
        require(tokenInAmount >= minimumAmount, 'Bay: Insignificant input amount');
        require(IERC20(tokenIn).allowance(msg.sender, address(this)) >= tokenInAmount, 'Bay: Input token is not approved');

        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), tokenInAmount);

        _swapAndStake(bayVault, tokenAmountOutMin, tokenIn, _pid);
    }

    function beefOut (address bayVault, uint256 withdrawAmount) external {
        (IBayVault vault, IPool pair) = _getVaultPair(bayVault);

        IERC20(bayVault).safeTransferFrom(msg.sender, address(this), withdrawAmount);
    ...
    function beefOutAndSwap(address bayVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        (IBayVault vault, IPool pair) = _getVaultPair(bayVault);
        address token0 = pair.token0();
        address token1 = pair.token1();
        require(token0 == desiredToken || token1 == desiredToken, 'Bay: desired token not present in liquidity pair');
    ...
    function _getVaultPair (address bayVault) private pure returns (IBayVault vault, IPool pair) {
        vault = IBayVault(bayVault);
        pair = IPool(vault.want());
    }

    function _swapAndStake(address bayVault, uint256 tokenAmountOutMin, address tokenIn, uint256 _pid) private {
        (IBayVault vault, IPool pair) = _getVaultPair(bayVault);
        require(pair.factory() == router.defaultFactory(), 'Bay: Incompatible liquidity pair'); // router.addLiquidity adds to pair from router.defaultFactory()

        (uint256 reserveA, uint256 reserveB,) = pair.getReserves();
        require(reserveA > minimumAmount && reserveB > minimumAmount, 'Bay: Liquidity pair reserves too low');

        bool isInputA = pair.token0() == tokenIn;
        require(isInputA || pair.token1() == tokenIn, 'Bay: Input token not present in liquidity pair');
        ...
        IERC20(vault).safeApprove(farm, vault.balanceOf(address(this)));
        IFarm(farm).deposit(_pid, vault.balanceOf(address(this)), msg.sender);
        _returnAssets(path);
    }
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            if (balance > 0) {
                if (tokens[i] == WETH) {
                    ...
                    require(success, 'Bay: ETH transfer failed');
                } else {
    ...
    function estimateSwap(address bayVault, address tokenIn, uint256 fullInvestmentIn) public view returns (uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        (, IPool pair) = _getVaultPair(bayVault);
        require(pair.factory() == router.defaultFactory(), 'Bay: Incompatible liquidity pair'); // router.addLiquidity adds to pair from router.defaultFactory()
    ...
    function bayInLP(IBayVault vault, uint256 _pid, uint256 _amount) public payable {
        if (msg.value > 0) {
            IWETH(WETH).deposit{value: msg.value}();
        }
        if(vault.want() == WETH){
            ...
            IERC20(vault.want()).safeTransferFrom(msg.sender, address(this), _amount);
        }
        IERC20(vault.want()).safeApprove(address(vault), 0);
        IERC20(vault.want()).safeApprove(address(vault), _amount);
        vault.deposit(_amount);
        IERC20(address(vault)).safeApprove(farm, 0);
        IERC20(address(vault)).safeApprove(farm, IERC20(address(vault)).balanceOf(address(this)));
        IFarm(farm).deposit(_pid, IERC20(address(vault)).balanceOf(address(this)), msg.sender);
    }
    function bayOutLP(IBayVault vault, uint256 _shares) public {
        ...
        if(vault.want() == WETH){
            ...
            IERC20(vault.want()).safeTransfer(msg.sender, IERC20(vault.want()).balanceOf(address(this)));
        }
