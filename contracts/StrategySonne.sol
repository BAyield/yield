...
contract StratFeeManagerInitializable is OwnableUpgradeable, PausableUpgradeable {
    ...
    address public bayFeeRecipient;

    uint256 constant DIVISOR = 1 ether;
    uint256 constant public WITHDRAWAL_FEE_CAP = 30;
    ...
    event SetBayFeeRecipient(address bayFeeRecipient);

    function __StratFeeManager_init(CommonAddresses calldata _commonAddresses, uint256 _totalFee) internal onlyInitializing {
        __Ownable_init();
        __Pausable_init();
        ...
        bayFeeRecipient = _commonAddresses.bayFeeRecipient;
        withdrawalFee = 0;
        totalFee = _totalFee;
    }
    ...
    // set new bay fee address to receive bay fees
    function setBayFeeRecipient(address _bayFeeRecipient) external onlyOwner {
        bayFeeRecipient = _bayFeeRecipient;
        emit SetBayFeeRecipient(_bayFeeRecipient);
    }

    function setTotalFee(uint256 _totalFee) external onlyOwner {
        require(_totalFee < 0.05 ether, "!max");
        totalFee = _totalFee;
    }

    function getWithdrawalFee() public view whenNotPaused returns(uint256) {
        return withdrawalFee;
    }

     // fetch fees from config contract
    function getFees() internal view returns (uint256) {
        return totalFee;
    }

    // fetch fees from config contract and dynamic deposit/withdraw fees
    function getAllFees() external view returns (uint256, uint256, uint256) {
        return (getFees(), depositFee(), withdrawFee());
    }
...
// File: contracts/BIFI/strategies/Velodrome/StrategyVelodromeGaugeV2.sol
...
contract StrategySonne is StratFeeManagerInitializable {
    ...
    event ChargedFees(uint256 callFees, uint256 bayFees);
    ...
    function initialize(
        ...
        __StratFeeManager_init(_commonAddresses, _totalFee);
        want = _want;
        gauge = _gauge;
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ICToken(gauge).redeemUnderlying(_amount - wantBal);
        ...
        uint256 withdrawalFeeAmount = wantBal * getWithdrawalFee() / WITHDRAWAL_MAX;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 callFeeAmount = nativeBal * totalFee / DIVISOR / 10;
        IERC20(native).safeTransfer(callFeeRecipient, callFeeAmount);

        uint256 bayFeeAmount = nativeBal * totalFee / DIVISOR * 9 / 10;
        IERC20(native).safeTransfer(bayFeeRecipient, bayFeeAmount);

        emit ChargedFees(callFeeAmount, bayFeeAmount);
    }

    // swap output and gets more want tokens.
    function addLiquidity() internal {
      uint256 outputBal = IERC20(output).balanceOf(address(this));
      if (native != output) {
        uint[] memory amounts = ISolidlyRouter(unirouter).getAmountsOut(outputBal, outputToWantRoute);
    ...
    function balanceOfPool() public view returns (uint256) {
        (,uint256 _amount, , uint256 _exchangeRate) = ICToken(gauge).getAccountSnapshot(address(this));
        return _amount * _exchangeRate / 1e18;
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        (,uint256 _amount, , ) = ICToken(gauge).getAccountSnapshot(address(this));
        return (IComptroller(comptroller).compSupplyState(gauge).index - IComptroller(comptroller).compSupplierIndex(gauge, address(this))) * _amount / 1e36 + IComptroller(comptroller).compAccrued(address(this));
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        return nativeOut * totalfee / DIVISOR / 10;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        ICToken(gauge).redeemUnderlying(balanceOfPool());
