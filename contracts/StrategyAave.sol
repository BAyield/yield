...
contract StrategyAave is StratFeeManagerInitializable {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IL2Pool(l2Pool).deposit(want, wantBal, address(this), 0);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IL2Pool(l2Pool).withdraw(want, (_amount - wantBal), address(this));
    ...
    function balanceOfPool() public view returns (uint256) {
        uint256 amount = IAToken(gauge).balanceOf(address(this));
        return amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        IL2Pool(l2Pool).withdraw(want, balanceOfPool(), address(this));
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(l2Pool, type(uint).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(l2Pool, 0);
    }
